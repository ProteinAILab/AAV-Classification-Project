...
class SaveModelCallback(Callback):
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        filename = f"epoch{epoch+1}_loss_{loss:.4f}_mae_{mae:.4f}_val_loss_{val_loss:.4f}_val_mae_{val_mae:.4f}.keras"
        filepath = os.path.join(self.directory, filename)
        self.model.save(filepath)
        # print(f"Model saved as {filepath}")            

class SaveModelCallback_class(Callback):
    def __init__(self, directory):
        super(SaveModelCallback_class, self).__init__()
        self.directory = directory
        if not os.path.exists(self.directory):
            os.makedirs(self.directory)
        self.filepath = None  # 初始化，避免 AttributeError
    ...
    def on_epoch_end(self, epoch, logs=None):
        ...
        filename = (
            f"epoch{epoch+1}_loss_{loss:.4f}_acc_{accuracy:.4f}"
            f"_val_loss_{val_loss:.4f}_val_acc_{val_accuracy:.4f}.keras"
        )
    ...
    def on_train_end(self, logs=None):
        ...
        if self.filepath is not None:
            print(f"Final model saved as {self.filepath}")
...
def parent_model(L1=140, L2=20):
    ...
    父模型构建了一个LSTM模型，该模型的参数适用于Fit4Function研究中的所有功能适应性模型。L1和L2定义了该模型两层的大小。
    """
    model = Sequential()
